...
-- Records table keeps metadata for uploaded exports and HTML content
create table if not exists records (
  id bigint generated by default as identity primary key,
  user_id uuid references profiles(id) on delete cascade,
  title text not null,
  path text, -- Optional for file storage
  content_type text default 'json' check (content_type in ('json', 'html')),
  html_content text, -- Store HTML content directly
  json_data jsonb, -- Store JSON data
  created_at timestamp with time zone default current_timestamp
);
